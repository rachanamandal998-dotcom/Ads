...
(function generateParticles() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    p.style.cssText = `
      ...
      bottom:${120 + Math.random() * 80}px;
      width:${size}px;
      height:${size}px;
      background:${colors[Math.floor(Math.random() * colors.length)]};
      --px:${(Math.random() - 0.5) * 80}px;
      --py:${-20 - Math.random() * 80}px;
      --pd:${1.5 + Math.random() * 3}s;
      --pde:${Math.random() * 4}s
    `;
...
// FOOD FLOAT ANIMATION VALUES
(function setFoodVars() {
  const foods = document.querySelectorAll('.food-item');
  foods.forEach((f) => {
    if (!f.style.getPropertyValue('--ft')) {
      f.style.setProperty('--ft', `${5 + Math.random() * 2.5}s`);
      f.style.setProperty('--fd', `${Math.random() * 1.5}s`);
      f.style.setProperty('--ri', `${-6 + Math.random() * 12}deg`);
      f.style.setProperty('--ro', `${-6 + Math.random() * 12}deg`);
      f.style.setProperty('--rm', `${-2 + Math.random() * 4}deg`);
    }
  });
})();
...
// Unmute on first click/tap/key anywhere
...
// HANDLE RESIZE - Regenerate road lines
let resizeTimer;
